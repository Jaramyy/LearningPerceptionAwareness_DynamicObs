...
- Throughout the repository, the name `PerceptionAwareDrone` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of PerceptionAwareDrone (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/PerceptionAwareDrone
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/PerceptionAwareDrone/PerceptionAwareDrone/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/PerceptionAwareDrone"
    ]
}
```
