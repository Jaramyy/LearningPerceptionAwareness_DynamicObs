...
class PIDPlotPublisher(Node):
    def __init__(self, name="vel_pid_pub"):
        ...
        self.pub_thrust_torque = self.create_publisher(Float32MultiArray, "/drone/cmd/thrust_torque", 10)
    ...
    def publish_thrust_torque(self, pub, thrust, torque):
        msg = Float32MultiArray()
        msg.data = [float(thrust), float(torque[0]), float(torque[1]), float(torque[2])]
        pub.publish(msg)
...
def main():
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            thrust, torque = controller.update_velocity_only_edit(
                pos_w=robot.data.root_pos_w,
                vel_w=robot.data.root_lin_vel_w,
                quat_w=robot.data.root_link_quat_w,   # (1,4)
                omega_b=robot.data.root_ang_vel_b,
            ...
            pid_publisher.publish_thrust_torque(pid_publisher.pub_thrust_torque, thrust.cpu().item(), torque.squeeze(0).cpu().numpy())
